...
tokenize <- function(corpus, n) {
  ...
  dt <- data.frame( term = dtm$dimnames$Terms, count = colSums(sM))
...
if (length(samples) < 3) {
  ...
  samples <- unlist(lapply(input, function(x) createSamples(x,.02)))
}
# 
# cleaning up
...
for (i in 1:length(documents)) {
  ...
  filtered <- filter(raw, profanity, dictionary)
  ...
  unigrams <- tokenize(corpus, 1)
  unigrams$doc <- d
  unigrams$colors <- colors[i]
  
  bigrams  <- tokenize(corpus, 2)
  bigrams$doc <- d
  bigrams$colors <- colors[i]

  trigrams <- tokenize(corpus, 3)
  trigrams$doc <- d
...
```{r, echo=F, fig.width=12, fig.height=4, message=F, warning=F}
# twitter_unique <- setdiff(all$twitter$unigrams$term, all$blogs$unigrams$term)
# twitter_unique <- setdiff(twitter_unique, all$news$unigrams$term)
# 
# blogs_unique <- setdiff(all$blogs$unigrams$term, all$twitter$unigrams$term)
# blogs_unique <- setdiff(blogs_unique, all$news$unigrams$term)
# 
# news_unique <- setdiff(all$news$unigrams$term, all$twitter$unigrams$term)
# news_unique <- setdiff(news_unique, all$blogs$unigrams$term)
# 
# ## differences in unigrams
# data.frame(length(twitter_unique), length(blogs_unique), length(news_unique))
# 
# head(twitter_unique, 50)
# head(blogs_unique, 50)
# head(news_unique, 50)

# plot_ngrams(unigrams$twitter, 20, "20 most frequent unigrams", "white")
# plot_ngrams(bigrams$twitter, 20, "20 most frequent bigrams", "white")
# plot_ngrams(trigrams$twitter, 20, "20 most frequent trigrams", "white")
# 
# plot_ngrams(unigrams$blogs, 20, "20 most frequent unigrams", "blue")
# plot_ngrams(bigrams$blogs, 20, "20 most frequent bigrams", "blue")
# plot_ngrams(trigrams$blogs, 20, "20 most frequent trigrams", "blue")
# 
# plot_ngrams(unigrams$news, 20, "20 most frequent unigrams", "red")
# plot_ngrams(bigrams$news, 20, "20 most frequent bigrams", "red")
# plot_ngrams(trigrams$news, 20, "20 most frequent trigrams", "red")
```
